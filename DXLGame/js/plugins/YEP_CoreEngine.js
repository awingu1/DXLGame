 ...
 * @param 中文模式的字体
 * @parent ---字体---
 * @desc 如果你的设置为中文模式,将用该字体。默认设置:GameFont,SimHei,Heiti TC,sans-serif。（rmmv都为默认模式,中文模式需要另调）
 * @default GameFont,SimHei, Heiti TC, sans-serif
 *
 * @param 日韩模式的字体
 * @parent ---字体---
...
Window_Base.prototype.drawActorLevel = function(actor, x, y) {
    ...
    var dw2 = this.textWidth(Yanfly.Util.toGroup(actor.maxLevel()));
        if (level > 99){
                this.drawText("大乘" + level +"层", x + dw1 - 3, y, 120, 'right');
        } else if (level > 89){
                this.drawText("渡劫" + level +"层", x + dw1 - 3, y, 120, 'right');
        } else if (level > 79){
                this.drawText("合体" + level +"层", x + dw1 - 3, y, 120, 'right');
        } else if (level > 69){
                this.drawText("化神" + level +"层", x + dw1 - 3, y, 120, 'right');
        } else if (level > 59){
                this.drawText("出窍" + level +"层", x + dw1 - 3, y, 120, 'right');
        } else if (level > 49){
                this.drawText("元婴" + level +"层", x + dw1 - 3, y, 120, 'right');
        } else if (level > 39){
                this.drawText("金丹" + level +"层", x + dw1 - 3, y, 120, 'right');
        } else if (level > 29){
                this.drawText("结丹" + level +"层", x + dw1 - 3, y, 120, 'right');
        } else if (level > 19){
                this.drawText("筑基" + level +"层", x + dw1 - 3, y, 120, 'right');
        } else if (level > 9){
                this.drawText("练气" + level +"层", x + dw1 - 3, y, 120, 'right');
        } else {
                this.drawText("凡体" + level +"层", x + dw1 - 3, y, 120, 'right');
        }
